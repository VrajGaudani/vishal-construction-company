...
async function createUserRole(req, res) {
  try {
    ...
    if (bodyData.id != null) {
      ...
      userRoleModule = await UserRoleModel.findOneAndUpdate(
        { _id: bodyData.id },
        { $set: setData },
        { new: true }
      ).lean();
      ...
      let findData = await UserRoleModel.findOne({ name: bodyData.name });
      if (findData) {
        // if data found check
        throw {
          status: false,
      ...
      userRoleModule = await new UserRoleModel(bodyData).save();
