...
const { AddressModel, ReqCallbackModel } = require("../../models");
...
const sendMessageToClient = async (bodyData) => {
  ...
  clientMessage +=
    "Your Request Received For Site Visit. We will get back to you at the earliest For any query Call " +
    mobile +
    " \n";
...
async function createCallbackRequest(req, res) {
  try {
    ...
    let findData = await ReqCallbackModel.findOne({ email: bodyData.email });
    if (findData) {
      // if data found check
      throw {
        status: false,
    ...
    let saveModule = await new ReqCallbackModel(bodyData).save();
