...
const { ServiceEnquiryModel, AddressModel } = require("../../models");
...
async function createServicesEnquiry(req, res) {
  try {
    ...
    let saveModule = await new ServiceEnquiryModel(bodyData).save();
    console.log("saveModule is", saveModule);
    if (saveModule) {
      await sendMessageToAdmin(bodyData);
      await sendMessageToClient(bodyData);
      await sendMail(bodyData);
    }
    res.send({
      status: true,
...
const sendMessageToClient = async (bodyData) => {
  ...
  clientMessage +=
    "Your Service Request Received. We will get back to you at the earliest For any query Call " +
    mobile +
    "\n";
