...
const clientMobile = async (bodyData) => {
...
const sendMessageToClient = async (bodyData) => {
  let clientDetails = await clientMobile(bodyData);
  console.log("clientDetails is", clientDetails);
  let clientMessage =
    "Hello " + clientDetails.firstName + " " + clientDetails.lastName + ", \n";
  clientMessage +=
    "Your Property " +
    bodyData.nameOfProject +
    ", Property Code:- " +
    bodyData.propertyCode +
    " submitted on Vishal Construction Company for approval, We will verify your request and revert you soon. Regards, Vishal Construction Company VISHCC \n";
...
async function createPropertyRequest(req, res) {
  try {
    ...
    let bodyData = _.pick(req.body, [
      ...
      "pricePerSqFt",
      "isPLCIncluded",
      "isCarParkingIncluded",
      "isClubMemberShipIncluded",
      "otherCharges",
    ...
    let propertiesLength = await PropertyModel.count();
    ...
    let saveModule = await new PropertyModel(bodyData).save();
    ...
    let featureSchemaModule = await new PFeaturesModel(
      moduleFeatureSchema
    ).save();
    console.log("saveModule is", featureSchemaModule);
    let priceSchemaModule = await new PPriceModel(priceSchema).save();
