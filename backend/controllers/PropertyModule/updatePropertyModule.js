...
const { PropertyModel, PFeaturesModel, PPriceModel } = require("../../models");
...
async function updatePropertyRequest(req, res) {
  try {
    ...
    let saveModule = await PropertyModel.findOneAndUpdate(
      { _id: bodyData.propertyId },
      { $set: bodyData }
    );
    let featureSchemaModule = await PFeaturesModel.findOneAndUpdate(
      { propertyId: bodyData.propertyId },
      { $set: moduleFeatureSchema }
    );
    let priceSchemaModule = await PPriceModel.findOneAndUpdate(
      { propertyId: bodyData.propertyId },
      { $set: priceSchema }
    );
