...
async function getAllPropertyForAdmin(req, res) {
  try {
...
async function getHomeAllProperty(req, res) {
  try {
    let LoginUser,
      myFavorite = [],
      allProperties = [];
    ...
    res.send({
      status: true,
      message: "Properties Data for Home Page",
      data: obj,
    });
  } catch (e) {
    console.log("Getting list err", e);
...
async function propertyDetail(req, res) {
  try {
    ...
    let findData = await PropertyModel.findOne({ _id }).lean();
    console.log("findData is", findData);
    if (findData) {
      let propertyFeaturesResponse = await PFeaturesModel.findOne({
        propertyId: findData._id,
      }).lean();
      let propertyImagesResponse = await PImageModel.findOne({
        propertyId: findData._id,
      }).lean();
      let propertyPriceResponse = await PPriceModel.findOne({
        propertyId: findData._id,
      }).lean();
      let propertyReviewResponse = await ReviewModel.find({
        propertyId: findData._id,
      })
        .sort({ _id: -1 })
        .lean();
      if (LoginUser && LoginUser != "") {
        let favQuery = {
          $and: [{ userId: LoginUser }, { propertyId: findData._id }],
        };
        isFavoriteData = await WishListModel.find(favQuery).lean();
      }

      property = await Promise.all([
        ...
        let totalRatingSum = propertyReview.reduce(function (
          totalSum,
          currentValue
        ) {
      ...
      res.send({
        status: true,
        message: "Property Note available.",
        data: findData,
      });
    }
  } catch (e) {
    ...
    await errorResponseHelper({
      res,
      error: e,
      defaultMessage: "Error in Getting Property Details",
    });
  }
}
...
async function deleteProperty(req, res) {
  try {
    ...
    let deleteData = await PropertyModel.remove({
      _id: req.body.propertyId,
    });
    let deleteFeaturesData = await PFeaturesModel.remove({
      _id: req.body.propertyId,
    });
    let deletePriceData = await PPriceModel.remove({
      _id: req.body.propertyId,
    });
    let deleteImageData = await PImageModel.remove({
      _id: req.body.propertyId,
    });
...
async function getPropertyLatLong(req, res) {
  try {
    ...
    let query = {
      $or: [{ "address.city": queryCity }, { "address.state": queryState }],
    };
    let data = await PFeaturesModel.find(query).lean();
...
async function getSearchTerms(req, res) {
  try {
    let data = await PFeaturesModel.distinct("address.city").sort();
...
async function getSearchMinMax(req, res) {
  try {
    let minPrice = await PPriceModel.aggregate([
      {
        $group: {
          _id: {},
    ...
    let maxPrice = await PPriceModel.aggregate([
      {
        $group: {
          _id: {},
    ...
    res.send({
      status: true,
      message: "Properties Min and Max Values",
      data,
    });
  } catch (e) {
    console.log("Getting list err", e);
