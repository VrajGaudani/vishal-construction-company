...
async function createOTP(req, res) {
  try {
    ...
    let ExpireAllOTP = await OtpModel.updateMany(
    ...
    let saveModule = await new OtpModel(dataToSave).save();
    if (saveModule) {
      ...
      res.send({
        ...
        smsResult: smsResult.statusText,
      });
    }
  } catch (e) {
    ...
    await errorResponseHelper({
      res,
      error: e,
      defaultMessage: "Error in Sending OTP",
    });
  }
}
